...
```{r}
ggplot(data = compare_response_time_data, aes(x =dependent , y = response_time, fill = dependent)) +stat_boxplot(geom = "errorbar", width = 0.2)+ geom_boxplot(outlier.color = "black")+ylab("Response Time (s)")+ stat_summary(geom = "point", shape = 4,fun = "mean", color = "black",size=2.8)+geom_beeswarm(cex = 1.2,show.legend = FALSE)+geom_signif(comparisons = list(c("no split", "split")),test = "t.test",na.rm = FALSE,map_signif_level = TRUE,y_position =22, col = "red") + theme_minimal()
```
