...
```{r}
hist(timestamp_no_split$reaction_time,breaks=seq(0,max(timestamp_no_split$reaction_time,timestamp_split$reaction_time)+10,5),xlab="Response Time (s)",main="Histogram of response time without split")
```
...
```{r}
hist(timestamp_split$reaction_time,breaks=seq(0,max(timestamp_no_split$reaction_time,timestamp_split$reaction_time)+10,5),xlab="Response Time (s)",main="Histogram of response time with split")
```
...
```{r}
ggplot(data = compare_response_time_data, aes(x =dependent , y = response_time, fill = dependent)) +stat_boxplot(geom = "errorbar", width = 0.2)+ geom_boxplot(outlier.color = "black")+ylab("Response Time (s)")+ stat_summary(geom = "point", shape = 2,fun = "mean", color = "black")+geom_beeswarm(aes(color = dependent),cex = 1.2,show.legend = FALSE)
```
